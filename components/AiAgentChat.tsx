...
function AiAgentChat({ videoId }: { videoId: string }) {
  ...
  const isVideoAnalysisEnabled = useSchematicFlag(FeatureFlags.ANALYSIS_VIDEO);
  ...
  useEffect(() => {
    ...
    switch (status) {
      ...
      case 'streaming':
        ...
        break;
      case 'error':
        toastId = toast('An error occurred while processing your request.', {
          id: toastId,
          icon: <BotIcon className='w-4 h-4' />,
        });
        break;
  ...
  const generateTitle = async () => {
    ...
    const userMessage: Message = {
      ...
      role: 'user',
      content: 'Generate a catchy title for this video!',
    };
    append(userMessage);
  };
